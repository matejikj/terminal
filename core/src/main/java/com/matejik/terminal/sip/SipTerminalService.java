...
public class SipTerminalService {
  ...
  public void connect(SipAccountCredentials credentials) {
    updateState(
        state -> state.withConnection(SipConnectionStatus.CONNECTING, Optional.of(credentials)));
  ...
  private void handleEvent(SipEvent event) {
    updateState(
        state ->
            switch (event) {
              case SipEvent.ConnectionChanged connectionChanged -> state.withConnection(
                  connectionChanged.status(), connectionChanged.account());
              case SipEvent.CallProgress callProgress -> state.withCallUpdated(callProgress.call());
              case SipEvent.CallTerminated callTerminated -> state.withCallTerminated(
                  callTerminated.call().id(), callTerminated.call());
            });
  }

  private void updateState(java.util.function.UnaryOperator<SipSessionState> updater) {
    stateStore.update(
        TerminalStateKeys.SIP_SESSION,
        state -> {
          if (state == null) {
