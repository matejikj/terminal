...
public abstract class CollectionService<T, ID> {
    ...
    protected CollectionService(RestClient restClient,
    ...
    public List<T> fetchItems(boolean forceRefresh) {
        ...
        var validCache = cached != null && !forceRefresh
        ...
        var response = restClient.get()
                ...
                .retrieve()
                .body(listType());
        var snapshot = new CollectionState<>(List.copyOf(response == null ? List.of() : response), Instant.now());
    ...
    private void mutateCache(Function<List<T>, List<T>> mutator) {
        stateStore.update(cacheKey, snapshot -> {
